...
l0 = [1.196549826581494 1.172604084137631 1.173956813721921 1.202012667688769]';
lde2pe = [0.359809755287430 0.363766765936637 0.361788142425923 0.361788142425923]';
omega = 2.5;
Ly = 0.1;
Lx = 0.1;
...
while count <= iter
    ...
    l_q = l0 + l_enc;
    w_d = pos_n*P;
    l_spool_to_pulley = sqrt(w_d.^2 + lde2pe.^2);
    ...
    time = toc(s_start);
    q_d         = [Lx*sin(omega*time);Ly*cos(omega*time)-0.1;0];
    q_d_dot     = [-Lx*omega*cos(omega*time); Ly*omega*sin(omega*time); 0];
    ...
    q_dotf = [ alpha(1) * q_dot(1) + (1-alpha(1))*q_dotf(1);
               alpha(2) * q_dot(2) + (1-alpha(2))*q_dotf(2);
               alpha(3) * q_dot(3) + (1-alpha(3))*q_dotf(3)];

    % Calculate Errors
    e           = q_d - q;
    e_dot       = q_d_dot - q_dotf;
...
%% Save logged data
% save("PlotData/PoseData.mat", "q_log","qd_log","e_log","e_int_log","f_log","T_friction_log","f_uncon_log","f_friction_log", "w_friction_log", "w_c_log", "timeVec", "vel_log", "fvel_log");
save("C:\Users\eliasoa\OneDrive - NTNU\Documents - Modellering, regulatordesign og simulering av kabeldrevet robot\General\TTK4900 Masteroppgave\Rapport\Tester og fremstillig av plots\MATLAB RUN\PlotData.mat", ...
    "q_log","qd_log","e_log","e_int_log","f_log","timeVec");
